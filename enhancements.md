...
- Onboarding flow (bottom sheet built, needs content/design refinement)
...
- ~~Hide the input composer on the Log Food review screen once foods are logged — the tray becomes a clean, scrollable review (reopen the sheet to add more), instead of the composer taking up space and forcing extra scroll.~~ - Completed ✅
...
- ~~onboarding flow to set personalised targets based on goals~~ - Completed ✅ (deployed 2026-06-08)
  - Formula-based personalised calorie + macro goals. User describes weekly activity in free text/voice → Gemini estimates an activity factor (5 bands) + resistance-training flag + structured workouts → Mifflin–St Jeor BMR (with 7% South Asian correction factor) → TDEE → goal-adjusted calories (rounded to 100) → macros (protein bodyweight-driven @1.0/1.5 g/kg, fat 27%, carbs remainder, fibre 14g/1000kcal). "Tell us about you" form (sex, DOB, height ft+in/cm, weight, weight goal, target kg/mo) → "Your personalized goals" review screen with editable calorie target (EditFood-style bar + pills) and a live maintenance-vs-target breakdown showing the deficit and projected kg/mo. Soft onboarding routing after signup; editable from Settings (ABOUT YOU card → form; Daily Goals card → review). Migrations `add-goal-personalization.sql` + `add-birth-date.sql` (both applied to live Supabase). Deferred follow-ups: safety floors (min-cal clamp/warning — do first), goal-aware color coding, adaptive TDEE (needs weight log), dynamic daily goals (uses the stored `activity_workouts`).
  - ~~South Asian BMR correction (0.93×) — Mifflin–St Jeor overestimates for Indian users due to higher avg body fat % and lower NEAT; applied a 7% reduction based on published RMR studies~~ - Completed ✅
  - ~~Form data retained on calculation failure — removed `resetForm()` from the catch block so the "About you" form keeps user input when the activity estimation API fails~~ - Completed ✅
  - ~~Default goals banner flicker fix — banner no longer flashes on page load for personalized-goals users; gated on `profileLoaded` flag~~ - Completed ✅
