...
- onboarding flow to set personalised targets based on goals
  - **BUILT 2026-06-08, pending user testing + deploy (not yet pushed to prod).** Formula-based personalised calorie + macro goals. User describes weekly activity in free text/voice → Gemini estimates an activity factor (5 bands) + resistance-training flag + structured workouts → Mifflin–St Jeor BMR → TDEE → goal-adjusted calories (rounded to 100) → macros (protein bodyweight-driven @1.0/1.5 g/kg, fat 27%, carbs remainder, fibre 14g/1000kcal). "Tell us about you" form (sex, DOB, height ft+in/cm, weight, weight goal, target kg/mo) → "Your personalized goals" review screen with editable calorie target (EditFood-style bar + pills) and a live maintenance-vs-target breakdown showing the deficit and projected kg/mo. Soft onboarding routing after signup; editable from Settings (ABOUT YOU card → form; Daily Goals card → review). Migrations `add-goal-personalization.sql` + `add-birth-date.sql` (both applied to live Supabase). Deferred follow-ups: safety floors (min-cal clamp/warning — do first), goal-aware color coding, adaptive TDEE (needs weight log), dynamic daily goals (uses the stored `activity_workouts`).
